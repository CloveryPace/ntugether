...
import { API_LOGIN, API_GET_PLAN_DETAIL } from '../global/constants';
...
export default function ReviewBox({id, question, need_reviewed, attendfuction}) {
      ...
      if(need_reviewed){
        ...
        axios.post(API_GET_PLAN_DETAIL + id + '/apply', "", config)
          ...
          .catch(function (err) {
              ...
              if (err.response.data === "Plan creator should not applied."){
                alert("參加者不可加入所創辦的活動");
              }
        });
      }
    ...
    return (
      ...
      :
        <Button variant="contained" color="primary" onClick={handleAttend}> {t("參加計畫")} </Button>
      }
      </>
    );
}