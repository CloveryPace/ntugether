...
import { API_GET_PLAN_DETAIL, API_GET_APPLICATION } from '../global/constants';
...
export default function PendingReview({id}) {
    ...
    const handleApprove = (apply_id) => { 
      ...
      //審核通過
      axios.patch(API_GET_PLAN_DETAIL + '/application/' + apply_id + '/approve',{ 
        "is_approved": true
      }, config)
        .then(function (res) {
            console.log(res);
    ...
    return (
      <Stack direction="column" spacing={1}>
        ...
        <div style = {component_2}>
            <Typography variant="h6">尚無申請資料</Typography>
        </div>
        }
      </Stack>
    );
